locals {
  ...
  vpc_id = try(aws_vpc.tfprojectvpc[0].id, "")
...
resource "aws_route" "public_internet_gateway" {
  ...
  gateway_id             = aws_internet_gateway.tfprojectvpc[0].id
...
resource "aws_internet_gateway" "tfprojectvpc" {
  count = local.create_public_subnets && var.create_igw ? 1 : 0
...
#   depends_on = [aws_internet_gateway.tfprojectvpc]
# }

# resource "aws_nat_gateway" "tfprojectvpc" {
...
#   nat_gateway_id         = element(aws_nat_gateway.tfprojectvpc[*].id, count.index)
